...
A {\em classical planning frame} is a tuple $\Phi=\tup{F,A}$, where $F$ is a set of fluents and $A$ is a set of actions. The dynamics of the actions in $A$ can be specified with two functions: $app(s)$ that denotes the subset of actions applicable in a state $s$ and $\theta(s,a)\}$ that denotes the {\em successor state} that results of applying $a$ in $s$.
...
\subsection{Observations of plan executions}
An observation $\tau$ of a plan $\pi$ executed for solving a given classical planning problem $P=\tup{F,A,I,G}$ is an interleaved combination of a sequence of {\em partially-observed} actions $\tup{a_1,\ldots,a_n}$ and a sequence of {\em partially-observed} states $\tup{s_1,\ldots,s_m}$ such that $n\leq |\pi|$ and $m\leq |\pi|+1$. Note that, because of the partial observability, the transitions between two consecutive observed states in $\tau$ may involve the execution of more than a single action. Formally $\theta(s_i,\tup{a_1,\ldots,a_k})=s_{i+1}$, where $k\geq 1$ is unknown and unbound. This means knowing $\tau$ does not mean knowing the actual length of $\pi$.

We say that the observation of a plan execution $\tau$ is {\em compliant} with a plan $\pi$ iff:
\begin{itemize}
\item The sequence of actions in $\tau$ is the same sequence of actions $\pi$ but with certain actions omitted.
\item The sequence of states in $\tau$ is the same sequence of states traversed by $\pi$ but (1), with certain states omitted and/or (2), the value of certain fluents omitted in that states.
\end{itemize}
...
\begin{itemize}
\item $P=\tup{F,A,I,G}$ is a classical planning problem such that the dynamics of the actions in $A$ is unknown (i.e. the $app(s)$ and $\theta(s,a)\}$ functions are unknown).
\item $M=\{\mathcal{M}_1,\ldots,\mathcal{M}_m\}$ is a finite and non-empty set of action models that define different semantics for the actions in $A$
\item $\tau$ is the observation of the execution of a solution plan for $P$.
\end{itemize}
A solution to the {\em model recognition} task is a discrete probability distributoin $P(\mathcal{M}|\tau)$ that express for each model $\mathcal{M}\in M$ its probability of producing $\tau$. Our approach is to estimate the $P(\mathcal{M}|\tau)$ probability, for each $\mathcal{M}\in M$, according to the amount of {\em edition} required by $\mathcal{M}$ to produce a plan that is an optimal solution for $P$ while it is compliant with $\tau$. 

\subsection{The space of \strips\ models}
In this work we assume that the dynamics of the actions in $A$ are specified using \strips\ action schemas. Like in PDDL~\cite{mcdermott1998pddl,fox2003pddl2}, we assume that fluents $F$ are instantiated from a set of {\em predicates} $\Psi$. Each predicate $p\in\Psi$ has an argument list of arity $ar(p)$. Given a set of {\em objects} $\Omega$, the set of fluents $F$ is induced by assigning objects in $\Omega$ to the arguments of predicates in $\Psi$, i.e.~$F=\{p(\omega):p\in\Psi,\omega\in\Omega^{ar(p)}\}$ s.t. $\Omega^k$ is the $k$-th Cartesian power of $\Omega$. We assume also that actions $A$ are instantiated from a set of action schemas that shape the preconditions and effects of the ground actions. Figure~\ref{fig:stack} shows the PDDL code for the {\em stack} actions, taken from a four-operator {\em blocksworld}~\cite{slaney2001blocks}.
\begin{figure}
\begin{scriptsize}
\begin{verbatim}
(:action stack
...
\begin{definition}
  Given $\mathcal{M}$, a \strips\ action model built from $F_v$ and $\tau$, an observation built with fluents in $F$ and actions in $A$. The {\bf observation edit distance}, $\delta(\mathcal{M},\tau)$, is the minimal edit distance from $\mathcal{M}$ to any {\em comparable} model $\mathcal{M}'$ s.t. $\mathcal{M}'$ produces an optimal plan $\pi^*$ s.t., $\tau$ is compliant with $\pi^*$; \[\delta(\mathcal{M},\tau)=\min_{\forall \mathcal{M}' \rightarrow \tau} \delta(\mathcal{M},\mathcal{M}')\]
\end{definition}
...
For each $\mathcal{M}\in M$, we compute the {\em observation edit distance} $\delta(\mathcal{M},\tau)$  using a classical planning compilation that follows these assumptions:
\begin{enumerate}
\item Observations in $\tau$ are {\em noiseless}. 
\item The initial state $I$ is {\em fully observed}. This means that the set of fluents $F$, and the corresponding set of predicates $\Psi$ that shapes the fluents in $F$, are inferrable from $I$.
\end{enumerate}
