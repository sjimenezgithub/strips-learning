...
The observation of a plan execution $\tau$ is {\em noiseless} iff the value of any fluent or any action observed in $\tau$ is correct. 
...
Given a classical planning problem $P=\tup{F,A,I,G}$ where the semantic of the actions in $A$ is unknown. $\tau$ the observation of the execution of a plan for solving $P$ and $M=\{\mathcal{M}_1,\ldots,\mathcal{M}_m\}$, a finite and non-empty set of action models that define different semantics for the actions in $A$, then {\em model recognition} is the task of identifying which model $\mathcal{M}\in M$ has the highest probability of producing $\tau$.

Our approach is to estimate this probability, for each $\mathcal{M}\in M$, according to the amount of {\em edition} required by $\mathcal{M}$ to produce a plan that is compliant with $\tau$. 
