...
\begin{center}
\begin{tikzpicture}
\node [thick,draw,circle] at (1,1.1) (a) {$q_0$};
\node [thick,draw,circle] at (4,1.1) (b) {$q_1$};
\node [thick,draw,circle] at (7,1.1) (c) {$q_2$};
\node [thick,draw,circle] at (10,1.1) (d) {$q_3$};
\node [thick,draw,circle] at (13,1.1) (e) {$q_4$};
\node [thick,draw,circle, below of=e]  (f) {$q_5$};
\draw (13,0.1) circle (.3cm);

\draw [thick,->] (a) to (b);
\draw [thick,->] (b) to (c);
\draw [thick,->] (c) to (d);
\draw [thick,->] (d) to (e);
\draw [thick,->, bend left] (e) to (a);
\draw [thick,->] (e) to (f);

\node at (2.4,1.5) {$\mathsf{take}(orange)$};
\node at (5.4,1.5) {$\mathsf{take}(knife)$};
\node at (8.4,1.5) {$\mathsf{split}(orange)$};
\node at (11.4,1.5) {$\mathsf{squeeze}(orange)$};
