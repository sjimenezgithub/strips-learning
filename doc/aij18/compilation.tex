...
\item $G_{\Lambda}=\bigcup_{1\leq i\leq n}\{test_i\}$, requires that the programmed action model is validated in the state observations $s_i\in\mathcal{T}$.
...
\item The length of $\mathcal{T}$, the observed plan execution. The larger $1\leq i\leq n$, the more $\{test_i\}$ fluents and $\{\mathsf{validate_{i}}\}$ actions in $P_{\Lambda}$.
\end{itemize}
