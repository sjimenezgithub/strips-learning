...
\item $\mathcal{O}=\tup{s_0,s_1,\ldots,s_{n}}$ is the sequence of {\em state observations} obtained watching the execution of an {\em unobserved} plan $\pi=\tup{a_1, \ldots, a_n}$ such that, for each {\small $1\leq i\leq n$}, $a_i$ is applicable in $s_{i-1}$ and generates the successor state $s_i=\theta(s_{i-1},a_i)$. We assume that the initial state $s_0$ is {\em fully observable} while the rest of states in $\mathcal{O}$ can be {\em partially observable}, meaning that some of the observed fluents are missing because it is unknown whether their value is either positive or negative. In the extreme, full state observations could be missing but we assume that state observations are {\em noiseless}, meaning that if the value of a fluent is observed it is correct. 
...
A solution to the $\Lambda=\tup{\mathcal{M},\mathcal{O},\Psi}$ learning task is a set of operator schema $\mathcal{M}'$ that is compliant with the input model $\mathcal{M}$, the given state observations $\mathcal{O}$ and the predicates $\Psi$. Solving this learning task implies determining, not only the \strips\ action model $\mathcal{M}'$, but also the unobserved plan $\pi$, that explains the input observations with the learned \strips\ model. Figure~\ref{fig:example-observations} shows an example of a $\Lambda=\tup{\mathcal{M},\mathcal{O},\Psi}$ task for learning the {\em blocksworld} \strips\ action model from the five-state observations sequence that corresponds to inverting a 2-block tower. Learning the action model from this example implies inferring that the unobserved plan is $\pi=\tup{\small\tt (unstack\ B\ A), (putdown\ B), (pickup\ A), (stack\ A\ B)}$.
...
Figure~\ref{fig:example-plans} shows an example of a learning task $\Lambda=\tup{\mathcal{M},\mathcal{O},\Psi,\pi}$, that corresponds to observing the execution of a four-action plan for inverting a two-block tower. The exaple skips $\mathcal{M},\Psi$ since they are the same as in Figure~\ref{fig:example-observations}. In this example $\mathcal{O}=\tup{s_0,s_4}$ which means that only the first and last states were observed and the three intermediate states $s_1$, $s_2$ and $s_3$ are fully unknown.
