...
\textcolor[rgb]{1.00,0.00,0.00}{Since we restrict our attention to learning \strips\ action models, we will assume the syntactic constraints imposed by \strips\ models, namely } $\eff^-(a)\subseteq \pre(a)$, $\eff^-(a)\cap \eff^+(a)=\emptyset$ and $\pre(a)\cap \eff^+(a)=\emptyset$. Additionally, actions $a\in A$ are instantiated from given action schemas, as in PDDL.
...
Plan traces can then be classified accordingly to the type of observed state trajectory (\FO, \POstar, \PO or \NO) and action sequence (\FO, \PO or \NO). %\textcolor[rgb]{1.00,0.00,0.00}{Section~\ref{task_definition} formalizes the addressed learning task according to the particular combination of observed state trajectories and observed action sequences.}
