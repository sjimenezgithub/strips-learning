...
A plan trace $\tau = \langle s_0, a_1, s_1, a_2, s_2, \ldots, a_n, s_n \rangle$ for a planning frame $\Phi=\tup{F,A}$ holds that $a_i\in A$ for every action in $\tau$ and that $s_i \in \mathcal{L}(F)$ for each {\small $1\leq i\le n$}.

NO TENGO CLARO LO QUE QUEREMOS DECIR CON ESTA FRASE (??). ADEMAS, NO ES CORRECTA PORQUE LOS ESTADOS DE UNA TRAZA PUEDEN SER FULL-OBSERVABLE Y $\mathcal{L}(F)$ SON PARTIAL ASSIGNMENTS OF VALUES TO FLUENTS.

=================


Plan traces can be classified accordingly to the type of observed state trajectory (\FO, \POstar, \PO or \NO) and action sequence (\FO, \PO or \NO). \textcolor[rgb]{1.00,0.00,0.00}{In section \ref{overview}, we expose the impact of all combinations of observed state trajectories and observed action sequences in the complexity of the learning task.}
