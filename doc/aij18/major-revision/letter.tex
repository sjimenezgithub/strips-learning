...
\textcolor[rgb]{1.00,0.00,0.50}{That depends on the domain we are trying to learn. Certainly, most planning domains do not need predicates with repeated variables, but in order to ensure the completeness of our learning approach we have opted to include it.}
...
\textcolor[rgb]{1.00,0.00,0.50}{\textbf{DIEGO ...  SE SIGUEN CUMPLIENDO LAS SYNTACTIC STRIPS CONSTRAINTS CON CONDITIONAL EFFECTS????? }}
...
\textcolor[rgb]{1.00,0.00,0.50}{Removing symmetries would admittedly improve performance for heuristic planners. This removal can be achieved by ensuring the actions are programmed following some order, which can be done via some additional fluents. We think, however, that the compilation scheme should remain as simple and general as possible to facilitate its understanding.
}

\textcolor[rgb]{1.00,0.00,0.50}{Nevertheless, we want to thank the reviewer for the great idea and we will look into adding this optimization to the approach as a compilation option.}
...
\textcolor{blue}{With respect to the repetition of the definition of a \strips\ action model, conditional effects are introduced in section 4 because these are exclusively used for modeling the task that results from the compilation, $P_{\Lambda}=\langle F_{\Lambda},A_{\Lambda},I_{\Lambda},G_{\Lambda} \rangle$. Nevertheless, we have now incorporated the explanation about conditional effects within the section 4.2 'Compilation' since this is a merely syntactic aspect of the task formalization.}
...
\textcolor{blue}{As indicated by the reviewer, timeouts are only observed in the setting with NO action sequences and NO state trajectories. We have added a few sentences providing more details on this issue.}

\textcolor[rgb]{1.00,0.00,0.50}{\textbf{SOLO HE VISTO UNA FRASE DE TIMEOUTS SOBRE EL GRID ... Y EL RESTO DE DOMINIOS? NO SE PUEDE DECIR NADA???? }}
...
\textcolor[rgb]{0.00,0.07,1.00}{In the remodeling of section 3, the paragraph about the size of the training dataset has been removed too. The ideas presented here have been moved to section 6.}

\textcolor[rgb]{1.00,0.00,0.50}{\textbf{DIEGO ... NO HE VISTO ESTE PARRAFO EN LA SECCION 6, NO PUSISTE ESTA INFORMACION EN 6??}}

\begin{mdframed}[hidealllines=true,backgroundcolor=gray!20]
- "Let $\Omega_v = {v_i}_{i=1}^{max_{a\in A} ar(a)}$ be a new set of objects "ì I think "$ar(a)$" should be "$|ar(a)|$"
\end{mdframed}

\textcolor[rgb]{1.00,0.00,0.50}{The function $ar(\cdot)$ returns the arity of an action or predicate (the length of its argument list) and is first introduced in the Background section.}
...
\textcolor[rgb]{1.00,0.00,0.50}{\textbf{DIEGO ...}}
...
\textcolor[rgb]{1.00,0.00,0.50}{\textbf{DIEGO ...  SE PUEDE DECIR ALGO MAS O IGNORAMOS EL COMENTARIO?}}
...
\textcolor[rgb]{1.00,0.00,0.50}{\textbf{DIEGO ...  HE A—ADIDO ESTA FRASE EN EL TEXTO PERO SOLO CON RESPECTO AL GTM. MIRA A VER SI TE PARECE BIEN.}}
...
\textcolor[rgb]{1.00,0.00,0.50}{\textbf{DIEGO ...  HE A—ADIDO ALGO PARA CLARIFICAR ESTO EN EL TEXTO PERO NO SE SI EST¡ BIEN ... COMPROBAR}}

\textcolor{blue}{Hopefully the new section 3 (see main modification \#2) helps the reviewer clarify this question. }
...
\textcolor{blue}{We keep adding new domains as we work on the approach so that number is representative of how many domains we have tested our approach on. We did, however, leave one domain out, {\em Sokoban}. As a matter of fact, this domain has some of the features more difficult to handle for our approach, those are puzzle-like domains and domains with grid-like structure.}
...
\textcolor{blue}{Predicates with arity 3 are certainly harder but still manageable. In order to proof this, we have added the {\em rovers} domain to the list of domains tested in section 6. }
