...
\textcolor{red}{All the changes are highlighted in red in the document file.}
...
We agree with the reviewer that the paper lacks a description of the overall approach. We refer the reviewer to the main modification \#1 exposed above. Following the notes of the reviewer's summary, we have also included a brief summary at the end of the section Introduction that serves to wrap up the main features of \FAMA. Additionally, and as requested by the other reviewers too, we introduced the main modification \#2. The new section 3 defines the learning task and provides insight of the nature of the problem to solve according to the type of the observed plan trace in section 3.1. We believe that this explanation also helps to grasp the overall picture of \FAMA.
...
The reviewer is right in that the definition of effects was confusing. It all stems from the adoption of the open world assumption, which leads to states that contain positive and negative literals, but we did not adapt the definition of effects appropriately. In section 2.1, we have now included a brief explanation of the open world assumption and the representation of states, and we have adapted the definition of effects accordingly.
...
Done. LOUGA is included in Table 2 and Table 3 and an explanatory paragraph about the main characteristics of LOUGA has been incorporated in section 2.2.
...
It depends on the domain we are trying to learn. Certainly, most planning domains do not need predicates with repeated variables, but in order to ensure the completeness of our learning approach we have opted to include it.
...
We refer the reviewer to the main modification \#3. Section 4 has been polished significantly. An introductory paragraph provides a high level description of the approach and examples have been incorporated throughout the text.
...
Removing symmetries would admittedly improve performance for heuristic planners. This removal can be achieved by ensuring the actions are programmed following some order, which can be done via some additional fluents. We think, however, that the compilation scheme should remain as simple and general as possible to facilitate its understanding.

Nevertheless, we want to thank the reviewer for the great idea and we will look into adding this optimization to the approach as a compilation option.
...
The implications can actually be encoded as a disjunction, as it is the case of the {\tt{\small apply\_stack}} action of Figure 4. The caption of the figure indicates that "implications are coded as disjunctions". The implication of the formula can then be encoded as $\{\neg pre_{p}(\xi) \vee p(\omega)\}_{\forall p\in\Psi_\xi}$.
...
All plotted values are averages over the 14 (now 15) tested domains. The reviewer is right, however, to point out that this is not said in the experiment of section 6.2, so we have added some lines making it explicit.
...
The take-home lesson of section 3 is that when the observed plan trace does not provide information of the solution plan length (case 2), planning is needed to find the solution and so the learning task is PSPACE-complete.
...
We acknowledge the reviewer's concern and refer the reviewer to the main modification \#1 exposed at the beginning of this response letter.
\vspace{0.1cm}
...
We have replaced the word 'abstract' by 'approximation' and rephrased the sentence a little bit.
...
With respect to the other two comments, besides using different expressions to refer to the two concepts, we have included some additional side explanations that we hope help understand the meaning of the sentences.
...
We have included an explanation of the open world assumption and how this affects the representation of states in Section 2.1.
...
The sentence was, indeed, unclear. Anyway, we removed it in the revised version of the paper since it was an expendable phrase that did not fit very well in this section either.
...
The reviewer is right in that \FAMA also assumes that predicates and action headers are provided or otherwise are extractable from the plan traces. We believe it is more appropriate to move this comment after LOCM is introduced since LOCM is an exception that learns the predicates using Finite-State Machines (FSM) because no observed states are given in the plan trace.
...
The figures that appear in the third column of Table 3 for each learning system were taken from the respective published papers. They all refer to the specific experimental setup used in the evaluation of the approaches. In general, since there is not a common benchmark for the evaluation of learning approaches, each system selects the configuration of training data that best suits the particularities of the approach. Hence, all the figures in the 3rd column of Table 3 refer to the specific implementation used in the experiments. What we tried to do in this section is to somehow compare the approaches by establishing some common elements of evaluation, namely number of tested domains, size of the training data in terms of action and/or traces, degree of observability, etc.
...
A sort is a group of objects found in the same position in the header of an action. Although this was already defined in the text, we have changed the font of the word to italics and we explicitly say now this set is "named \emph{sort"}.
...
We refer the reviewer to the main modification \#2. Section 3 has substantially changed and this sentence does no longer appear in the text.
...
We agree with the reviewer that relevant results have recently been achieved in the setting of full observability. Now we acknowledge, in Section 3.1. of our paper, the work by Stern and Juba that allows computing the least number of examples that are necessary to learn models within a given error rate.
...
We have removed the paragraph regarding the evaluation of the learned models from section 3 and moved to section 5. We also clarify the use of the word "semantic" as requested by another reviewer as well.

\begin{mdframed}[hidealllines=true,backgroundcolor=gray!20]
- "On the other hand, a striking figure ..."  - I could not understand this sentence.
\end{mdframed}

In the remodeling of section 3, the paragraph about the size of the training dataset has been removed as well. The ideas presented here have been moved to page 23, at the end of section 6.2.

\begin{mdframed}[hidealllines=true,backgroundcolor=gray!20]
- "Let $\Omega_v = {v_i}_{i=1}^{max_{a\in A} ar(a)}$ be a new set of objects "ì I think "$ar(a)$" should be "$|ar(a)|$"
\end{mdframed}

The function $ar(\cdot)$ does not return a set, it returns the arity of an action or predicate; i.e., the length of its argument list. This is introduced in the Background section for the first time.
...
The size of the space of possible \strips\ models for a given $\xi$ is $2^{2|\Psi_{\xi}|}$ because the value of two bits is enough to code how a given element from $\Psi_{\xi}$ appears in a given action model $\xi$: (0,0) indicates it does not appear in $\xi$, (1,0) indicates it appears as precondition, (1,1) indicates it appears as precondition and a negative effect and (0,1) indicates it appears as a positive effect.
...
We have added a new sentence that overviews the post-process. We have also rephrased some part of the text to make it more comprehensible. We believe the text provides now a clear idea of the post-process. Nevertheless, we refer the reviewer to the pseudo-code presented in [40] (algorithm 2) for a more detailed explanation.
...
We have provided better justification for the use of semantic evaluation metrics in section 5.
...
Yes, the reviewer is right in that we put it all in the planner and get the overall plan, both building actions and validating actions. What we mean by "solvable in two steps" is that a SAT-based planner internally solves the construction of the model in two \emph{time steps}: in one time step the truth value of all the model fluents that represent the preconditions is set, and in a second time step the value of all the model fluents that represent the effects is set. 

%We have slightly modified the sentence to make it clear we refer to the SAT solving process of Madagascar.
...
Yes, precision gives a notion of soundness since it measures how many of the learned preconditions and effects are correct. We have improved the explanation of the metrics in section 5 to make this more clear.
...
We have replaced the word "complete" by "adequate" as suggested by the reviewer.
...
We agree with the reviewer's observation, and we believe the misunderstanding stems from us mixing up the words \emph{information} and \emph{observability}. In our claim of \FAMA using the minimal amount of information, we should actually say that \FAMA uses the \emph{minimal observability} possible, being the extreme case to have no actions at all and only the initial state and final state as observations. In this respect, LOCM is indeed the learning approach that requires the \emph{minimal information} since no state or predicate is needed. We want to point out, however, that \FAMA does not require the number of steps that are in the training plan, and we hope the new section 3 (see main modification \#2) does a better job at conveying this point.

We think that LOCM is a learning approach that notably differs from the rest of systems presented in Table 2 and, in this regard, we have highlighted this distinction in the explanation of section 2.2. Besides adding a sentence that stresses the ability of LOCM to learning with minimal input knowledge, we devote the last paragraph of section 2.2. to explain the distinctive features of the LOCM family.
...
The syntactic metrics commonly used by other approaches count the number of differences wrt the GTM and normalize by the total number of possible preconditions and effects. This gives rise to an optimistic measure of the error since the set of possible preconditions and effects is usually much larger than the size of the actual GTM, resulting in low error rates even when the model is largely flawed. In contrast, precision and recall normalize by the size of the GTM, thus removing this problem. Furthermore, precision and recall provide an intuitive notion of soundness and completeness, respectively; whereas a sound model is interpreted as a model in which all preconditions and effects are correct wrt the GTM, a complete model is one in which no precondition or effect is missing. We have added some paragraphs to section 5 to better explain these concepts.

As the reviewer points out, \FAMA will create a model that explains all the training traces because it is a satisfying approach (that satisfies all the training samples) and so the training error is 0. There is, however, no guarantee that the training traces are able to capture the whole dynamics of the domain. This is specially true for scenarios with low observability like the ones explored in this paper. Hence, it is always interesting to measure how well the learned model is able to explain a new set of plan traces.
...
The semantic evaluation is used to measure the ability of a learned model to produce a given plan trace. In this context, the use of the word "semantic" refers to the physics of the domain, not the actual meaning of the predicates or actions. We have added a comment clarifying this point.

We agree with the reviewers concerns about the definitions of INS, DEL and size(M) and have corrected them.

Following the reviewer recommendation, we have splitted section 5.1 into two sections: one for the syntactic evaluation and another for the semantic evaluation.

\begin{mdframed}[hidealllines=true,backgroundcolor=gray!20]
  - I advise changing title Experimental Results $=>$ Experimental Evaluation
\end{mdframed}

We changed the title as recommended by the reviewer.
...
Predicates with arity 3 are certainly harder but still manageable. In order to prove this, we have added the {\em rovers} domain to the list of domains tested in section 6. 
...
In the NO / NO case study, the number of action steps is not known. The changes introduced in main modification \#3 should make the distinction between known and unknown horizon more explicit.
