...
\textcolor{blue}{A sort is a group of objects found in the same position in the header of an action. Although this was already defined in the text, we have changed the font of the word to italics and we explicitly say now this set is "named \emph{sort"}.}
...
\textcolor{blue}{We have added a new sentence that overviews the post-process. We have also rephrases some part of the text to make it more comprehensible. We believe the text provides now a clear idea of the post-process and that the pseudo-code is not necessary. Nevertheless, we refer the reviewer to the pseudo-code presented in [40] (algorithm 2) for a more detailed explanation.}
...
\textcolor{blue}{We agree with the reviewer's observation, and we believe the misunderstanding stems from us mixing up the words \emph{information} and \emph{observability}. In our claim of \FAMA using the minimal amount of information, we should actually say that \FAMA uses the \emph{minimal observability} possible, being the extreme case to have no actions at all and only the initial state and final state as observations. In this respect, LOCM is indeed the learning approach that requires the \emph{minimal information} since no state or predicate is needed.}

\textcolor{blue}{We think that LOCM is a learning approach that notably differs from the rest of systems presented in Table 2 and, in this regard, we have highlighted this distinction in the explanation of section 2.2. Besides adding a sentence that stresses the ability of LOCM to learning with minimal input knowledge, we devote the last paragraph of section 2.2. to explain the distinctive features of the LOCM family.}
