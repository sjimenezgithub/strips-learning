...
When the input plan trace contains \PO observed actions, the extra conditional effects
$\{at_{i},plan(name(a_i),\Omega^{ar(a_i)},i)\}\rhd\{\neg at_{i},at_{i+1}\}_{\forall i\in [1,n]}$ are included in the $\mathsf{apply_{\xi,\omega}}$ actions to ensure that actions are applied in the same order as they appear in $\tau$. \textcolor{red}{If the sequence of actions is FO, then this conditional effect is redefined as regular preconditions and effects in the apply actions. Doing so will make it impossible for the planner to execute apply actions other than the observed actions in the plan trace. Furthermore, for traces with \POstar or \FO state trajectories, we force that only one apply action is executed between two observed states by adding the precondition $\neg action_{applied}$ in every apply action.}
