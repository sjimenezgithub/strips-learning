...
For instance, Figure~\ref{fig:grid-example} illustrates a $\Phi$-observation where $\Phi$ only contains fluents of the kind {\tt\small (x-coord ?v)} and {\tt\small (y-coord ?v)}. This means that, for each observed state, only the value of fluents {\tt\small (x-coord ?v)} and {\tt\small (y-coord ?v)} is known while the value of the fluents {\tt\small (next ?v1 ?v2)}, {\tt\small (state-q0)} and {\tt\small (state-q1)} is unknown.
...
We first formalize \strips\ schemas, then define the full space of possible \strips\ schema and eventually, we introduce an {\em edit distance} which allows us to estimate the $P(\tau|\mathcal{M})$ likelihoods for observations of the execution of classical plans and planning \strips\ models.
...
\strips\ action schema provide a compact representation for specifying classical planning models. For instance, Figure~\ref{fig:model-example} shows six \strips\ action schema codded in PDDL for a robot navigation in a $n\times n$ grid. A \strips\ action schema $\xi$ with name, $name(\xi)$, defines a list of {\em parameters} $pars(\xi)$, and the three list of predicates ($pre(\xi)$, $del(\xi)$ and $add(\xi)$) that shape the kind of fluents that can appear in the {\em preconditions}, {\em negative effects} and {\em positive effects} of the actions induced from that schema. 
