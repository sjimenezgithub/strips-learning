...
;
  ...
  \end{center}
  \end{scriptsize}  
 \caption{\small {\em Bayesian network} abstracting that model $\mathcal{M}'$ results from editing $\mathcal{M}$ and that can produce a plan $\pi$ consistent with $\tau$.}
\label{fig:net}
\end{figure}

\subsection{Approximating the $P(\tau|\mathcal{M})$ {\em likelihood}}
Intuitively, our approach to estimate the $P(\tau|\mathcal{M})$ {\em likelihood} is to assess how far is $\mathcal{M}$ from a model $\mathcal{M'}$ that can produce a plan $\pi$ that solves $P$ and s.t. $\tau$ is a {\em consistent} observation of the execution of $\pi$ on $P$. Figure~\ref{fig:net} shows the four-variable {\em Bayesian network} abstracting that $\mathcal{M}'$ is a model that results from editing the input model $\mathcal{M}$ s.t. $\mathcal{M}'$ can produce a solution plan $\pi$ consistent with $\tau$. Regarding this network, a natural way to formulate the aimed {\em likelihood} is
\[
 P(\tau|\mathcal{M})=\sum_{\mathcal{M}'}\sum_{\pi}\frac{P(\mathcal{M},\mathcal{M}',\pi,\tau)}{P(\mathcal{M})},
\]
...
Given that the joint probability distribution can be marginalized as $\displaystyle P(\mathcal{M},\mathcal{M}',\pi,\tau)=P(\mathcal{M})P(\mathcal{M'}|\mathcal{M})P(\mathcal{\pi}|\mathcal{M'})P(\tau|\mathcal{\pi})$, that $P(\tau|\mathcal{\pi})=1$ for plans consistent with $\tau$ and that $P(\tau|\mathcal{\pi})=0$ for $\tau$ inconsistent plans. Likewise, $P(\mathcal{\pi}|\mathcal{M'})=0$ for plans that cannot be generated with a model $\mathcal{M'}$, i.e. for {\em invalid plans}~\cite{howey2004val}. We have that
\[
P(\tau|\mathcal{M})=\sum_{\mathcal{M}'}\sum_{\pi} P(\mathcal{M'}|\mathcal{M})P(\mathcal{\pi}|\mathcal{M'}),
\]
where $\mathcal{M}'$ now ranges over the models that can synthesize {\em valid plans} and $\pi$ ranges only over the valid plans for $\mathcal{M}'$ consistent with $\tau$.
