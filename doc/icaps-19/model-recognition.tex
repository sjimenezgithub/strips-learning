...
\begin{tikzpicture}[scale=.4]
  ...
  \end{center}
  \end{scriptsize}  
 \caption{\small Plan execution of a robot navigating a $5\times 5$ grid.}
...
\begin{figure}
  ...
  (:action inc-x
    :parameters (?px1 ?px2)
    :precondition (and (x-pos ?px1) (next ?px1 ?px2) (even))
    :effect (and (not (x-pos ?px1)) (x-pos ?px2)))

  (:action dec-x
    :parameters (?px1 ?px2)
    :precondition (and (x-pos ?px1) (next ?px2 ?px1) (odd))
    :effect (and (not (x-pos ?px1)) (x-pos ?px2)))

  (:action inc-y-min-odd
    :parameters (?py1 ?py2)
    :precondition (and (y-pos ?py1) (not (next ?py2 ?py1)) (odd))
    :effect (and (not (y-pos ?py1)) (y-pos ?py2) (not (odd)) (even)))

  (:action inc-y-min-even
    :parameters (?py1 ?py2)
    :precondition (and (y-pos ?py1) (not (next ?py2 ?py1)) (even))
    :effect (and (not (y-pos ?py1)) (y-pos ?py2) (not (even)) (odd)))

  (:action inc-y-max-odd
    :parameters (?py1 ?py2)
    :precondition (and (y-pos ?py1) (not (next ?py1 ?py2)) (odd))
    :effect (and (not (y-pos ?py1)) (y-pos ?py2) (not (odd)) (even)))

  (:action inc-y-max-even
    :parameters (?py1 ?py2)
    :precondition (and (y-pos ?py1) (not (next ?py1 ?py2)) (even))
    :effect (and (not (y-pos ?py1)) (y-pos ?py2) (not (even)) (odd)))
  \end{verbatim}           
  \end{tiny}  
 \caption{\small Action model for a robot navigation in a $5\times 5$ grid.}
...
This paper introduces the task of {\em model recognition}.  Given a partially observed plan execution and a set of possible planning models, {\em model recognition} is the task of identifying which model in the set has the highest probability of producing the input observation. To better illustrate {\em model recognition}, imagine a robot in a grid whose navigation is determined by the \strips\ model shown in Figure~\ref{fig:model-example}. According to this navigation model the robot increments its {\em x-coordinate} when it is at an {\em even} row while, at {\em odd rows}, the {\em x-coordinate} is decremented. Further, the robot only increments the {\em y-coordinate} either at the end or at the begining of rows. Apart from this particular navigation model, different models can be defined within the same state variables and these models can determine different kinds of robot navigation. Given an observation of a plan execution (e.g, the six-step plan illustrated by Figure~\ref{fig:grid-example}) the {\em model recognition} task aims here to identify which model is more likely of producing the given observation. 
...
A {\em classical planning frame} is a tuple $\Phi=\tup{F,A}$, where $F$ is a set of fluents and $A$ is a set of actions whose dynamics are specified with two functions: $App(s)$ that denotes the subset of actions applicable in a state $s$ and $\theta(s,a)$ that denotes the {\em successor state} that results of applying $a$ in $s$. In this work we are specifying action dynamics with the \strips\ model. With this regard, an action $a\in A$ is defined with:
\begin{itemize}
...
Here we analyze the particular instantiation of the {\em model recognition} task where the dynamics of the actions $A$ (i.e. the $App$ and $\theta$ functions) are specified with \strips\ action schemas. Figure~\ref{fig:model-example} shows the PDDL code for the action schema of the mentioned navigation model (Figure). 
