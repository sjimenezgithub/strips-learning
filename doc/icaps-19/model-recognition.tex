...
%the actual applied transitions are unknown as well as the internal machine state. Assuming that the actual applied transitions is unknown means that the observation $\mathcal{O}(\tau)$ of the execution of a regular automata contains no actions, it is simply a sequence of states $\mathcal{O}(\tau)=\tup{s_1, \ldots, s_m}$. Assuming that the internal machine state is unknown means that $\mathcal{O}(\tau)$ is a $\Phi$-observation and that the $\Phi$ subset does not contain {\small\tt ($q$)} fluents, with $q\in Q$.
...
;
  ...
  \end{center}
  \end{scriptsize}
 \caption{\small Four-symbol four-state {\em regular automata} for recognizing the $(abc)^+$ language (\underline{$q_3$} is the acceptor state).}
\label{fig:regautomatae}
\end{figure}

Table \ref{tab:conf_matrix} shows the confusion matrix resulting from classifying the 100 strings. In this matrix, rows represent the real class and columns the predicted class. Even though we are using a suboptimal planner, we can see that all strings were correctly labeled. These results proof that our approach is able to perform classic classification tasks.
...
\begin{table}[hbt!]
	...
	\end{tabular}
	\caption{Confusion matrix of the regular automata classification task.}
	\label{tab:conf_matrix}
\end{table}


\subsubsection{Recognition of navigation models.} In this experiment the given models in $M$ represent different navigation models, that is, different ways to navigate a $N\times N$ grid. In particular, we have constrained the applications of the navigation actions producing different navigation policies e.g. like the one in Figure~\ref{fig:model-example}. 

Given an observation of a plan execution, like the one illustrated at Figure~\ref{fig:grid-example}, here the task is to identify which navigation model produced that observation, despite the the applied actions being unobserved. In addition, for each observed state, only the value of fluents encoding the x and y coordinates of the agent are known ($\Phi =$\{xcoord, ycoord\}) while the value of the hidden factors conditioning the navigation policy are unknown.
