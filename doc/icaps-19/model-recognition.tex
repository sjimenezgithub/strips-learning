...
\subsubsection{Full observability of the executed plan.} The {\em full observability of the executed plan} is a too strong assumption for {\em model recognition} but it allows us to build reasonable estimates of the $P(\mathcal{O}|\mathcal{M})$ {\em likelihood} for the general case.
...
If there is a single possible trajectory and a single possible model $\mathcal{M}^*$ consistent with the input observation then, the probabilities of expression (1) are not added up so expression (1) simplifies to:
\begin{align}
...
\subsubsection{Partial observability of the executed plan.} In a similar way, an approximation of the $P(\mathcal{O}|\mathcal{M})$ {\em likelihood} can be built for the general case where the executed plan is partially observed. To deal with the new scenario we add the following two assumptions:
\begin{enumerate}
...
\end{enumerate}
Similar assumptions were taken in previous work for {\em goal recognition}~\cite{ramirez2012plan}. They allow us to claim that $\sum_{\tau}$ in equation (2) is dominated by its largest term, so that probabilities corresponding to different trajectories that reach the goals of $P$ (and are consistent with the given observation) are not added up. Likewise, $P(\mathcal{O}|\tau*)=\alpha$, where $\tau^*$ is the single trajectory that corresponds to the single optimal plan $\pi^*$ for $P$ (consistent with $\mathcal{O}$ and $\alpha$ is a normalizing constant referred to $|\pi^*|$ (the number of possible observations of a bounded trajectory is bounded too).

Again, there is also a single model $\mathcal{M^*}$ that can produce that single trajectory $\tau*$. As a consequence, when there is {\em partial observability of the executed plan}, and under the previous assumptions, expression (1) simplifies again to expression (3)
...
\end{align}
where $N$ is the length of the string, and $p$ is $\frac{3}{4}$.
...
\begin{align}
P(\mathcal{M'}|\mathcal{M}) = 0.75^d \times 0.25^{N-d}
\end{align}
where $d=\delta(\mathcal{M},\mathcal{M}')$. Likewise we can formulate $P(\mathcal{M^*}|\mathcal{M})$ with this same equation but now, the parameter $d$ is given by the {\em observation distance}, $d=\delta(\mathcal{M},\mathcal{O})$.
