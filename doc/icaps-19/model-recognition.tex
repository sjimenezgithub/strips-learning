...
\item we will assume that the length of the trajectory is known so as to keep the possible explanations bounded and force the optimal explanation of $\mathcal{O}(\tau)$; more precisely, at least one fluent of every state of $\mathcal{O}(\tau)$ is observed and the length of the plan trajectory is thus fixed by the observation
\end{itemize}
...
Figure \ref{fig:blocks_acc} shows the {\em accuracy} of our approach when identifying failures over 30 different observations. The positive trend seen in the figure proves that the more complete the observations are the more accurate our estimation becomes. 
