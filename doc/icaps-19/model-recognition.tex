...
(:action inc-x
  ...
  \end{verbatim}           
  \end{scriptsize}  
 \caption{\small Programmable action {\tt\small inc-x(?v1,?v2)} for robot navigation in a $n\times n$ grid.}
...
\begin{enumerate}
\item $\Psi$, is the set of {\em state predicates}. A set of FOL predicates that shape the propositional state variables.
\item $pars(\xi)$, the list of {\em parameters} of that schema. 
\end{enumerate}

For each action schema $\xi$, given the set of {\em state predicates} $\Psi$, then $F_{\xi}$ defines the subset of elements that can appear in the preconditions and effects of that action schema. This set is the same contains ten elements for the mentioned navigation model, {\small\tt\{x-coord($v_1$), x-coord($v_2$), y-coord($v_1$), y-coord($v_2$), odd(), even(), next($v_1$,$v_1$), next($v_1$,$v_2$), next($v_2$,$v_1$), next($v_2$,$v_2$))\}}.
