...
The first experiment is a proof-of-concept \emph{classification task} that perfectly recognizes the regular automata of a given string. Additionally, the two assumptions underpinning our formal theory are experimentally proved in the second experiment by varying the degree of observability. Finally, the robustness of the approach is shown in the third experiment where actions and intermediates states are unobservable. A significant lesson from the experiments is that our approach allows us to predict behaviours that are not ruled by a goal-driven deliberative planning model (e.g., models of fixed programs or finite state machines --regular automata-).
