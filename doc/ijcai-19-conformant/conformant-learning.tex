...
(del_holding_v1_stack) (del_clear_v2_stack)
(add_clear_v1_stack) (add_handempty_stack) (add_on_v1_v2_stack)
  \end{verbatim}           
  \end{tiny}  
 \caption{\small Propositional encoding of the {\tt\small stack(?v1,?v2)} schema.}
\label{fig:propositional}
\end{figure}



      \item The set of actions $A_c$ contains now actions of two different kinds:
\begin{itemize}
...
\end{small}
      ...
      \item Actions whose semantics is given by the value of the $[pre|del|add]\_e\_\xi$ fluents at the current state. Given an operator schema $\xi\in\mathcal{M}$ its {\em editable} version is formalized as:
\begin{small}  
\begin{align*}
...
(:action stack
  ...
  :effect
   (and (when (del_on_v1_v1_stack) (not (on ?o1 ?o1)))
        (when (del_on_v1_v2_stack) (not (on ?o1 ?o2)))
        (when (del_on_v2_v1_stack) (not (on ?o2 ?o1)))
        (when (del_on_v2_v2_stack) (not (on ?o2 ?o2)))
        (when (del_ontable_v1_stack) (not (ontable ?o1)))
        (when (del_ontable_v2_stack) (not (ontable ?o2)))
        (when (del_clear_v1_stack) (not (clear ?o1)))
        (when (del_clear_v2_stack) (not (clear ?o2)))
        (when (del_holding_v1_stack) (not (holding ?o1)))
        (when (del_holding_v2_stack) (not (holding ?o2)))
        (when (del_handempty_stack) (not (handempty)))
        (when (add_on_v1_v1_stack) (on ?o1 ?o1))
        (when (add_on_v1_v2_stack) (on ?o1 ?o2))
        (when (add_on_v2_v1_stack) (on ?o2 ?o1))
        (when (add_on_v2_v2_stack) (on ?o2 ?o2))
        (when (add_ontable_v1_stack) (ontable ?o1))
        (when (add_ontable_v2_stack) (ontable ?o2))
        (when (add_clear_v1_stack) (clear ?o1))
        (when (add_clear_v2_stack) (clear ?o2))
        (when (add_holding_v1_stack) (holding ?o1))
        (when (add_holding_v2_stack) (holding ?o2))
        (when (add_handempty_stack) (handempty))))
  \end{verbatim}           
  \end{tiny}  
 \caption{\small Editable version of the {\tt\small stack(?v1,?v2)} schema.}
\label{fig:editable}
\end{figure}
