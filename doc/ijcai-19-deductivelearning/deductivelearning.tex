...
We define the task of learning an action model from {\em state-invaraints} as a tuple $\Lambda=\tup{F,I,G,\Phi,M}$, where:
\begin{itemize}
\item $F$ is a state of fluents, $I$ is an initial state (a full assigment of the fluents), and $G$ is a set of goal conditions over the fluents.
...
A {\em solution} to a $\Lambda=\tup{F,I,G,\Phi,M}$ learning task is a model $\mathcal{M}'\in M$ that is {\em consistent} with the given input knowledge. This means that, let $P=\tup{F,A_{\mathcal{M}'},I,G}$ be a {\em classical planning problem} where the dynamics of the actions is given by the action model $\mathcal{M}'$, there exists a plan $\pi$ such that any state traversed in the trajectory $\tau(\pi,P_\mathcal{O})$ satisfies all the {\em state-invariants} in $\Phi$.
...
%They compute an action model that maximizes some notion of {\em statistical consistency} over a set of observations of plan executions so output an action model that is {\em consistent} with the input knowledge.
