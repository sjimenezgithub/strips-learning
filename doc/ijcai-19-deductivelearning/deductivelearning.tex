...
The goals of the classical planning problem output by the original compilation are extended with the $\neg mode_{inval}$ literal to validate that only states {\em consistent} with the state constraints defined in $\Phi$ are traversed by the solution plans. The $\neg mode_{inval}$ literal allows us also to define a more compact definition $\mathsf{apply_{\xi,\omega}}$ actions than the one previously proposed by~\citeauthor{aineto2018learning}~\citeyear{aineto2018learning} that do not require a disjunction to code the possible preconditions of an action schema.
