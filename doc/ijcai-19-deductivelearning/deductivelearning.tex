...
This section evaluates the improvement of using domain-specific knowledge for learning action models.
...
In our first experiment we seek to answer the question of whether domain knowledge can substitute observations. To this end, we evaluate the following 4 settings:

\begin{itemize}
	\item \textbf{Minimal observation:} This is the baseline setting where the learning example is reduced to the minimum and only the initial state and part final state are known ($\sigma = 0$ w/o $\Phi$).
	\item \textbf{Only $\Phi$:} Here we add domain knowledge encoded as schematic mutexes to the baseline scenario ($\sigma = 0$ with $\Phi$).
	\item \textbf{Only observability:} In this one, we use a more complete observation where intermediate states are partially observed ($\sigma = 0.2$ w/o $\Phi$).
	\item \textbf{Both observability and $\Phi$:} In the last setting we use both more complete observations and schematic mutexes ($\sigma = 0.2$ with $\Phi$).
\end{itemize}
...
Table \ref{tab:observability_vs_knowledge} compiles the average precision (P) and recall (R) for each domain in the different settings tested. The table also reports the number of schematic mutexes ($|\Phi|$) used for each domain. Comparing the settings wtih only domain knowledge and only observability (second and third columns), it is clear that having more complete learning examples is preferable. In fact, the improvement of using domain knowledge is marginal with respect to the baseline case. This is not the same for the last setting, where the use of domain knowledge shows a significant improvement in the quality of the learned models when compared to the setting with only observability.
