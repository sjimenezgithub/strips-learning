...
The set of learning examples is fixed for all the experiments so we can evaluate the effect of the amount of input knowledge in the quality of the learned models. %All learning examples are generated solving a small problem from the mentioned repository.
...
\begin{footnotesize}
	...
	\end{center}
\end{footnotesize}        
\caption{\small {\em Precision} and {\em recall} of the learned models.}
\label{tab:results_plans}
\end{table}

Table~\ref{tab:time_plans} reports the total planning time, the preprocessing time (in seconds) invested by {\sc Madagascar} to solve the classical planning instances that result from our compilation as well as the number of actions in the solutions found. Remarkably all the learning tasks are solved in a few seconds time.	
