...
The size of the classcial planning task $P_{\Lambda}$ output by the compilation depends on:
\begin{itemize}
\item The arity of the actions headers in $\mathcal{M}$ and the predicates $\Psi$ given as input of the $\Lambda$ learning task. The larger these numbers, the larget the $F_v(\xi)$ sets, that define the $pre_f(\xi)/del_f(\xi)/add_f(\xi)$ fluents and the corresponding actions for {\em programming} operator schema $\xi\in\mathcal{M}$.
