...
\begin{enumerate}
\item Is defined as a planning compilation. This fact opens the door to the {\em bootstrapping} of planning action models and allows us to report results over a wide range of planning domains. 
...
\item Can estimate the probability distribution of the possible \strips\ models, given an observation sequence. This extension allow us to address model recognition tasks as well as to evaluate the quality of the learned models semantically, without comparing with a reference model.
\end{enumerate}
...
Finally we define $F_v(\xi)\subseteq F_v$ as the subset of elements that can appear in a given action schema $\xi$. For instance, for the {\em stack} action schema $F_v({\tt stack})=F_v$ while $F_v({\tt pickup})$={\small\tt\{handempty, holding($v_1$), clear($v_1$), ontable($v_1$), on($v_1,v_1$)\}} only contains the fluents from $F_v$ that do not involve $v_2$.
...
\end{verbatim}
\end{scriptsize}
 \caption{\small Example of a task for learning a \strips\ action model in the blocksworld from a sequence of five state observations.}
\label{fig:lexample}
\end{figure}
...
\item Validates the \strips\ action model $\Xi'$ in $\mathcal{O}$. The solution plan continues with a postfix that produces the sequence of states in $\mathcal{O}$ starting from $s_0$, and using the programmed action model $\Xi'$. 
\end{enumerate}
...
\begin{itemize}
\item Fluents $F$ built instantiating the predicates $\Psi$ with the objects appearing in the input observations $\mathcal{O}$. 
...
\end{itemize}
\item $I_{\Lambda}$ contains the fluents from $F$ that encode $s_0$ (the first observation) and every $pre_f(\xi)\in F_{\Lambda}$ and $mode_{prog}$ set to true. Our compilation assumes that initially operator schemes are programmed with every possible precondition, no negative effect and no positive effect.
...
\begin{small}
The compilation forces that once the preconditions of an operator schema $\xi \in \Xi$ are programmed, they cannot be altered. The same happens with the positive and negative effects (furthermore, effects can only be programmed after preconditions are programmed). Once operator schemes are programmed they can only be applied because of the $mode_{prog}$ fluent. To solve $P_{\Lambda}$, goals $\{test_i\}$, {\small $1\leq i\leq n$} can only be achieved: executing an applicable sequence of programmed operator schemes that reaches the state $s_i$, starting from $s_0$. If this is achieved for every {\small $1\leq i\leq n$}, it means that the programmed action model $\Xi$ is compliant with the provided input knowledge and hence, solves $\Lambda$.
\end{small}
\end{proof}


\begin{lemma}
Completeness. Any \strips\ action model $\Xi$ computable from $\Lambda=\tup{\Psi,\Xi,\mathcal{O}}$ can be obtained by solving the corresponding classical planning task $P_{\Lambda}$.
\end{lemma}

\begin{proof}[Proof sketch]
\begin{small}
By definition, $F_v(\xi)\subseteq F_\Lambda$ fully captures the set of elements that can appear in a \strips\ action schema $\xi\in\Xi$. Any possible \strips\ action schema built with the fluents in $F_v$ can be computed because the compilation only constrains the application of $\mathsf{apply_{\xi,\omega}}$ actions iff they are not compliant with the $\mathcal{O}$ sequence.
\end{small}
\end{proof}
...
\begin{itemize}
\item {\em Deletion}. A fluent $pre_f(\xi)/del_f(\xi)/add_f(\xi)$ is removed from the operator schema $\xi$
\item {\em Insertion}. A fluent $pre_f(\xi)/del_f(\xi)/add_f(\xi)$ is added to the operator schema $\xi$
\end{itemize}

Now we are ready to define a metric that asseses then how dissimilar two given \strips\ action models are
\begin{definition}
The {\bf \strips\ edit distance}, denoted as $\delta(\Xi,\Xi')$, is the minimum number of edit operations that allow to transform a given \strips\ action model $\Xi$ into the given \strips\ action model $\Xi'$. This edit distance satisfies the {\em metric axioms} provided that the two operations has the same positive cost.
\end{definition}
...
\begin{definition}
The maximum {\bf \strips\ edit distance} is an upperbound, denoted as $\delta(\Xi,*)=\sum_{\xi\in\Xi} 3|F_v(\xi)|$, that indicates the maximum distance required to transform a given \strips\ action model $\Xi$ into any \strips\ action model $\Xi'$. 
\end{definition}

In more detail, for a given operator schema $\xi$ the maximum number of edits that can be introduced to their precondition set is $|F_v(\xi)|$. With regard to action effects, the maximum number of edits that can be introduce to their precondition set is $2|F_v(\xi)|$.

Likewise we can define the edit distance with regard to a sequence of observations $\mathcal{O}$>
\begin{definition}
 
\end{definition}


\subsection{Recognition and evaluation of \strips\ models}
...
\subsection{Recognition with classical planning}
