...
\begin{abstract}
This paper presents a classical planning compilation for learning \strips\ action models from observations of plan executions. Remarkably the compilation is fed with {\em state-constraints}, that bound the space of possible action models, and does not require the precise actions in the plan executions. A plan that solves the classical planning task resulting from this compilation induces a \strips\ action model that is compliant with the given state-constraints.  When the amount of input knowledge is too scarce, e.g. the state-constraints are too loose, evaluating learned \strips\ models with respect to reference action model becomes hard. The paper introduces a novel evaluation methodology that allows to asses the quality of learned \strips\ models even if actions are reformulated. Last but not least, the paper shows the flexibility of our compilation to include further information about the observed plan executions and increase the accuracy of the learned models.   
\end{abstract}
...
\item Does not require the precise actions applied in the observed plan executions but is extensible to further information about the observed plan executions to improve the learned models.
\item Assesses the quality of learned \strips\ models with a reference model even when the learning hypothesis space is so low constrained that the learned actions models can be reformulated and still be compliant with the inputs. %For instance in the {\em blocksworld}, given a low amount of input knowledge, operator {\small\tt stack} could be {\em learned} with the preconditions and effects of the {\small\tt unstack} operator (and vice versa) making non trivial evaluating the learned model with respect to a reference model.
\end{enumerate}
...
A {\em state invariant} is a formula $\phi$ that holds in the initial state of a given classical planning problem, $I\models \phi$, and at any state reachable from that state, i.e. for all states $s$ reachable from $I$ it holds that $s\models \phi$. State invariants are traditionally useful for making satisfiability planning and backward search more efficient~\cite{rintanen2014madagascar,alcazar2015reminder}.

A {\em mutex} (meaning mutually exclusive) is a particular case of state invariant that takes the form of a binary clause and that represents a pair of different properties that cannot be simultaneously true~\cite{kautz:mutex:IJCAI1999}. For instance in a three-blocks blocksworld, $on(block1,block2)$ and $on(block1,block3)$ are mutex because $block1$ can only be on top of a single block.

A {\em lifted invariant} (also called schematic invariants in the literature) is a state invariant defined using a first order formula over the predicates of a given planning problem~\cite{rintanen:schematicInvariants:AAAI2017}. A {\em domain invariant} is a state invariant that holds for any possible initial state, they are instance-independent and often, they are lifted invariants~\cite{fox:TIM:JAIR1998}.  For instance in the blocksworld, $\forall x,y,z:\ (on(x,y)\wedge on(x,z)) \implies y=z$, is a domain and lifted invariant because a block can only be on top of a single block no matter the initial state of the classical planning problem and no matter the particular identity of the blocks.
...
\subsection{Compiling the learning of \strips\ action models into classical planning}
...
\end{itemize}
\item $I_{\Lambda}$ contains the fluents from $F$ that encode $s_0^1$ (the initial state of the first label) and every $pre_f(\xi)\in F_{\Lambda}$ and $mode_{prog}$ set to true. Our compilation assumes that initially any operator schema is programmed with every possible precondition, no negative effect and no positive effect.
...
\subsection{State constraints}
Every state invariant $\phi\in\Phi$ is added as an extra precondition of actions $\mathsf{apply_{\xi,\omega}}$ for {\em applying} an already programmed operator schema. Likewise every state invariant $\phi\in\Phi$ must hold at the last reached state so they are included aas extra goals in $G_{\Lambda}$.

With regard to state trajectories. Now $\Sigma=\{\sigma_1,\ldots,\sigma_{\tau}\}$ is not a set of $(initial, final)$ state pairs but a set of state trajectories $\sigma_t=(s_0^t,s_1^t,\ldots,s_{n}^t)$, {\tt\small $1\leq t\leq \tau$}, that comprises the sequence of states resulting from executing the {\em unobserved} plan $\pi_t=\tup{a_1^t, \ldots, a_n^t}$ starting from the {\em initial} state $s_0^t$.

\begin{itemize}
\item Fluents, $at_j$ and $next_{j,j_2}$, {\small $1\leq j<j2\leq n$}, are added to $F_{\Lambda}$ with the aim of iterating through state trajectories. Likewise $I_{\Lambda}$ includes also now the fluents $at_1$ and $\{next_{j,j_2}\}$, {\small $1\leq j<j2\leq n$} 
\item Actions $\mathsf{apply_{\xi,\omega}}$ for {\em applying} an already programmed operator schema have an extra conditional effect $\{at_{j}\}\rhd\{\neg at_{j},at_{j+1}\}_{\forall j\in [1,n]}$. Likewise actions $\mathsf{validate_{t}}$ for {\em validating} a learning example {\tt\small $1\leq t\leq \tau$} add $at_1$ and delete $at_{|\pi_t|}$.
\end{itemize}


\subsection{Plan constraints}

Here we augment the input knowledge with the actions executed by the observed agent, when available. So the leaerning task is now defined as $\Lambda=\tup{\Psi,\Sigma,\Phi,\Pi}$
