...
;;; Actions corresponding to non-deterministic effects
  ...
  (:action stack-fail4
	     :parameters (?x ?y ?z)
	     :precondition (and (holding ?x) (clear ?y) (on ?y ?z))
	     :effect (and (not (on ?y ?z)) (clear ?z) (ontable ?y) (ontable ?x) (not (holding ?x)) (clear ?x) (handempty)))
	    
  (:action unstack-fail2
	     :parameters (?x ?y ?z)
	     :precondition (and (on ?x ?y) (clear ?x) (handempty) (on ?y ?z))
	     :effect (and (holding ?x) (clear ?y) (clear ?z) (ontable ?y) (not (clear ?x)) (not (handempty)) (not (on ?x ?y)) (not (on ?y ?z))))
